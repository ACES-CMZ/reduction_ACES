...
for ch in `seq 0 ${incr} 300`; do
    ch1=$ch
    ch2=$((${ch1} + ${incr}))
    jobid=$(sbatch --job-name=aces_cs21_mosaic_ch${ch1}to${ch2} \
        ...
        --wrap "/blue/adamginsburg/adamginsburg/miniconda3/envs/python39/bin/python -c \"from aces.imaging.mosaic_12m import make_giant_mosaic_cube_cs21; make_giant_mosaic_cube_cs21(channels=range(${ch1},${ch2}), skip_final_combination=True, verbose=True,)\"")
    jobids+=$jobid+":"
done

echo "Job IDs are ${jobids}"

sbatch --job_name=aces_cs21_mosaic_merge \
