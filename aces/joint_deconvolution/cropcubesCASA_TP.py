...
imagenames = find_files(path, '*.cube.I.sd.fits')
imagenames.sort()
...
for imagename in tqdm(imagenames, desc='Processing images'):
    ...
    for i in range(len(lines)):
        ...
        hdu = crop_cube_velocity_range(imagename, frequency, v_start=-10, v_end=160, v_res=3)

        if hdu == None: 
            continue

        # Define the output filename
        outputfile = imagename.replace('.fits', '.%s.fits' %process_string(line))
