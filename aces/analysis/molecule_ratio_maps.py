...
molecules = ['CS21', 'HC3N', 'HCOP', 'HNCO_7m12mTP', 'SiO21', 'SO21']
...
def make_ratio(mol1, mol2, suffix):
    f1 = f'{basepath}/mosaics/cubes/moments/{mol1}_CubeMosaic_{suffix}.fits'
    f2 = f'{basepath}/mosaics/cubes/moments/{mol2}_CubeMosaic_{suffix}.fits'
    ...
    if np.all(fh2[0].data.shape == fh1[0].data.shape):
        ...
        out = f'{basepath}/mosaics/cubes/moments/ratios/{mol1}_over_{mol2}_{suffix}.fits'
        fh1.writeto(out, overwrite=True)
        print(f"wrote {mol1}_over_{mol2}_{suffix}")


if __name__ == '__main__':
    os.makedirs(f'{basepath}/mosaics/cubes/moments/ratios', exist_ok=True)
