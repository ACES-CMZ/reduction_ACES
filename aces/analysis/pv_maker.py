...
def make_pv_b(cube, b, mol):
    ...
    pv_mean.write(f'{save_path}/{mol}_pv_b{round(b.value, 3)}_mean.fits')
    pv_max = subcube.max(axis=1)
    pv_max.write(f'{save_path}/{mol}_pv_b{round(b.value, 3)}_max.fits')
...
def make_pv_l(cube, l, mol):
    ...
    pv_mean.write(f'{save_path}/{mol}_pv_l{round(l.value, 3)}_mean.fits')
...
def make_pv_mol(cube_fn):
    ...
    for b in list_b:
        print('Making PV diagram at b =', b)
        make_pv_b(cube, b, mol)

    for l in list_l:
        print('Making PV diagram at l =', l)
