...
if __name__ == "__main__":
    ...
    if not dodask:
        ...
        cube = SpectralCube.read(f'{molname}_CubeMosaic.fits')
        print(cube)

        print(f"PV peak intensity.  dt={time.time()-t0}", flush=True)
        pv_max = cube.max(axis=1, how='slice')
        pv_max.write(f"{molname}_CubeMosaic_PV_max.fits", overwrite=True)

        print(f"PV mean.  dt={time.time()-t0}")
        pv_mean = cube.mean(axis=1, how='slice')
        pv_mean.write(f"{molname}_CubeMosaic_PV_mean.fits", overwrite=True)

        print(f"mom0.  dt={time.time()-t0}")
        mom0 = cube.moment0(axis=0, how='slice')
        mom0.write(f"{molname}_CubeMosaic_mom0.fits", overwrite=True)

        print(f"max.  dt={time.time()-t0}")
        mx = cube.max(axis=0, how='slice')
        mx.write(f"{molname}_CubeMosaic_max.fits", overwrite=True)

        print(f"PV max 2.  dt={time.time()-t0}")
        pv_max = cube.max(axis=2, how='slice')
        pv_max.write(f"{molname}_CubeMosaic_PV_b_max.fits", overwrite=True)

        print(f"PV mean 2.  dt={time.time()-t0}")
        pv_mean = cube.mean(axis=2, how='slice')
        pv_mean.write(f"{molname}_CubeMosaic_PV_b_mean.fits", overwrite=True)
        ...
        cube = SpectralCube.read(f'{molname}_CubeMosaic.fits', use_dask=True)
        print(cube)
